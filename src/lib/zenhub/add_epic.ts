...
export const addEpic = async (args: Args): Promise<void> => {
  const {organization, repository, issueNumber, epicRepository, epicIssueNumber} = args
  const {id: repositoryId} = await getRepository(organization, repository)
  const {id: epicRepositoryId} = await getRepository(organization, epicRepository)
  await fetchZenHub(
    "POST",
