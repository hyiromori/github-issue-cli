...
export const getBoard = async (organization: string, repository: string, workspaceId: string): Promise<Board> => {
  const {id: repositoryId} = await getRepository(organization, repository)
  return await fetchZenHub("GET", `/p2/workspaces/${workspaceId}/repositories/${repositoryId}/board`)
}
